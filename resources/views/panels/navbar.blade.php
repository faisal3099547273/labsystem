...
<div class="navbar-container d-flex content">
  ...
  <ul class="nav navbar-nav align-items-center ms-auto">
    ...
    @php
     $count = 0;
    // $count = \App\Models\UserNotification::where('user_id', auth()->id())
    //     ->where('status', '0')
    //     ->count();
    // $notifictions = [];
    // if ($count > 0) {
        ...
        <ul class="dropdown-menu dropdown-menu-media dropdown-menu-end">
            <li class="dropdown-menu-header">
                <div class="dropdown-header d-flex">
                    ...
                    <div class="badge rounded-pill badge-light-primary">{{ @$count }} New</div>
                </div>
            </li>
            <li class="scrollable-container media-list">
                {{-- @foreach (@$notifictions as $value)
                @php
                $img = 'custom/img/no-img.jpg';

                    if($value->notifications->user->is_company == true){
                 
                       $img = 'company/logo/'.@$value->notifications->user->company->logo;
                    }

                    if($value->notifications->user->is_busines == true){
                      $img = 'company/cover-photo/'.@$value->notifications->user->busines->logo;
                    }

                    if($value->notifications->user->is_busines == true){
                      $img = 'company/logo/'.@$value->notifications->user->busines->logo;
                    }

                    if($value->notifications->user->is_candidates == true){
                      if(@$value->notifications->user->avatar){
                        $img = 'user-img/'.@$value->notifications->user->avatar;
                      }
                      // echo 'sss';
                    }
    ...
    <li class="nav-item dropdown dropdown-user">
      <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);"
        ...
        <span class="avatar">
          ...
          $img = 'images/avatars/male.png';
          if(auth()->user()->is_company == true){
            $img = 'company/logo/'.auth()->user()->company->logo;
          }
          if(auth()->user()->is_candidates == true || auth()->user()->is_admin == true){
            if(auth()->user()->avatar){
              $img = 'user-img/'.auth()->user()->avatar;
            }
      ...
      if(auth()->user()->is_candidates == true){
        $route = 'candidate.profile';
      }

      if(auth()->user()->is_busines == true){
        $route = 'busines.profile';
      }
